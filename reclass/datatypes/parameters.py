...
class Parameters(object):
    ...
    def interpolate(self, inventory=None):
        ...
        if self._resolve_errors.have_errors() and not self._settings.ignore_missing_references:
    ...
    def _interpolate_render_value(self, path, value, inventory):
        try:
            ...
            if self._settings.ignore_missing_references:
                new = value.plain_text()
                print >>sys.stderr, "[WARNING] Reference '%s' undefined" % (new)
            else:
                new = None
